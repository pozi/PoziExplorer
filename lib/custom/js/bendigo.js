...
var showGarbageLayer = function(a){
	showOrHideLayer("Garbage Collection (Commercial)",true);
	showOrHideLayer("Garbage Collection (Residential)",true);
	showOrHideLayer("Organic Collection (Residential)",true);
}

// No passed parameter
var hideGarbageLayer = function(){
	showOrHideLayer("Garbage Collection (Commercial)",false);
	showOrHideLayer("Garbage Collection (Residential)",false);
	showOrHideLayer("Organic Collection (Residential)",false);
}
...
var highlightRoute = function(b){
	...
	var myRouteStore = new Ext.data.JsonStore({
		...
		baseParams:{
			...
			shapeFormat:'raw',
			generalize:0,
			key:decodeURIComponent('Fmjtd%7Cluu821utll%2Cb0%3Do5-94bs9w')
		},
